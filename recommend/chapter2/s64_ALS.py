...
from recommend.utils import evaluate
...
class ALS():
    ...
    def backward(self, r, r_pred, u, i, lr, lamda):
        ...
        :param lamda: 正则项系数
        """
        loss = r - r_pred  # r_pred为PQ + bu + b1
        self.p[u] += (loss * self.q[i] - lamda * self.p[u]) * lr
        self.q[i] += (loss * self.p[u] - lamda * self.q[i]) * lr
        self.bu[u] += (loss - lamda * self.bu[u]) * lr
        self.bi[i] += (loss - lamda * self.bi[i]) * lr
