...
from recommend.chapter2 import s34_UserCF_01label as userCF
import collections
import numpy as np
...
def trainUserCF(user_items_train, user_pos_items, k=5):
    ...
    recommendations = get_recomedations_by_usrCF(user_sims, user_items_train, user_pos_items)
    return recommendations


def trainItemCF(user_items_train, item_users_train, k=5):
    item_sims = knn4dict(item_users_train, k)
    recommendations = get_recomedations_by_itemCF(item_sims, user_items_train)
    return recommendations
